        ...
        for task in "${tasks[@]}"; do
            ...
            for ratio in "${ratios[@]}"; do
                ...
                for alpha in "${alphas[@]}"; do
                    ...
                    model=${model} \
                    model.model_args.pretrained_model_name_or_path=src/tv/tofu_${model}/${task}/ratio${ratio}/${forget_split}_tv_alpha_${alpha} \
                    retain_logs_path=saves/finetune/tofu_${model}_${retain_split}/evals/TOFU_EVAL.json \
                    paths.output_dir=src/tv/tofu_${model}/${task}/ratio${ratio}/${forget_split}_tv_alpha_${alpha}/evals

                    target_dir=src/tv/tofu_${model}/${task}/ratio${ratio}/${forget_split}_tv_alpha_${alpha}
