import torch
from datasets import AutoModelForCausalLM
...
rrf_model_id = "yourpath/saves/unlearn/tofu_Llama-3.1-8B-Instruct/tofu_Llama-3.1-8B-Instruct_forget01_GradDescent_retain" # Can change to forget05, forget10
...
with torch.no_grad():
    for (name, param_A), (_, param_B) in zip(rf_model.named_parameters(),
        ...
        V_retain_dict[name] = (param_B.cpu() - param_A.cpu())


final_save_path = "yourpath/src/tv/tofu_Llama-3.1-8B-Instruct/tofu_make_rank/pre_weight/forget01_llama_8b_it_retain_vector.pt" # Can change to forget05, forget10
torch.save(V_retain_dict, final_save_path)
